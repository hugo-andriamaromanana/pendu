...
scoreboard_button_rect_center = scoreboard_button_text.get_rect(
    center=scoreboard_button_rect.center)
...
scoreboard_popup_exit_button_rect_center = scoreboard_popup_exit_button_text.get_rect(
    center=scoreboard_popup_exit_button_rect.center)
# Title text
...
click_me_button_rect_center = click_me_button_text.get_rect(
    center=click_me_button_rect.center)
# Easy button exit
Easy_button_exit_rect = pygame.Rect(650, 500, 200, 50)
Easy_button_exit_text = COMIC_SANS.render("X", True, WHITE)
Easy_button_exit_rect_center = Easy_button_exit_text.get_rect(
    center=Easy_button_exit_rect.center)
# Title better luck next time
title_better_luck_next_time_rect = pygame.Rect(50, 590, 900, 50)
title_better_luck_next_time_text = COMIC_SANS.render(
    end_messages[count_for_end_message], True, BLACK)
title_better_luck_next_time_rect_center = title_better_luck_next_time_text.get_rect(
    center=title_better_luck_next_time_rect.center)
# Input_box_text
...
input_name_box_text = COMIC_SANS.render(
    'Please enter your name: '+(' '.join(input_name)), True, BLACK)
input_name_box_rect_center = input_name_box_text.get_rect(
    center=input_name_box_rect.center)
# ------------Game_window-------------------
...
def game_state(state):
    ...
    if state == "main_menu":
        ...
        screen.blit(title_better_luck_next_time_text,
                    title_better_luck_next_time_rect_center)
        DISPLAYSURF.blit(pygame.image.load(
            "hangman.png").subsurface(sub_surface), (200, 250))
        ...
        pygame.draw.rect(scoreboard_popup, RED,
                         scoreboard_popup_exit_button_rect)
        scoreboard_popup.blit(scoreboard_popup_exit_button_text,
                              scoreboard_popup_exit_button_rect_center)
        scoreboard_popup.blit(COMIC_SANS.render(
            "SCOREBOARD", True, BLUE), (400, 10))
        scoreboard_popup.blit(COMIC_SANS.render(
            "Easy", True, YELLOW), (100, 100))
        scoreboard_popup.blit(COMIC_SANS.render(
            "Medium", True, ORANGE), (450, 100))
        ...
        for i, level in enumerate(levels):
            ...
            for j, position in enumerate(positions):
                ...
                scoreboard_popup.blit(
                    COMIC_SANS.render(text, True, BLACK), (x, y))
    elif state == "Hard":
        DISPLAYSURF.blit(pygame.image.load("hangman.png").subsurface(
            game_vars["sub_surface"]), (200, 250))
        DISPLAYSURF.blit(COMIC_SANS.render(
            "Difficulty = Hard", True, RED), (400, 10))
        DISPLAYSURF.blit(COMIC_SANS.render(
            "Guess the word!", True, GREEN), (400, 100))
        DISPLAYSURF.blit(COMIC_SANS.render(
            ' '.join(game_vars['word_to_guess_display']), True, BLACK), (500, 400))
        DISPLAYSURF.blit(COMIC_SANS.render("Guesses left: " +
                         str(game_vars['lives']), True, BLACK), (100, 500))
        DISPLAYSURF.blit(COMIC_SANS.render(
            "Time : " + parse_time(time.time() - start), True, (0, 0, 0)), (500, 500))
        DISPLAYSURF.blit(COMIC_SANS.render(
            "Letters used: "+", ".join(game_vars['used_keys']), True, BLACK), (100, 600))
        DISPLAYSURF.blit(COMIC_SANS.render(
            "Score: "+str(game_vars['score']), True, BLACK), (500, 600))
        game_vars = game_time(game_vars, events)
    elif state == "Medium":
        DISPLAYSURF.blit(pygame.image.load("hangman.png").subsurface(
            game_vars["sub_surface"]), (200, 250))
        DISPLAYSURF.blit(COMIC_SANS.render(
            "Difficulty = Medium", True, ORANGE), (400, 10))
        DISPLAYSURF.blit(COMIC_SANS.render(
            "Guess the word!", True, GREEN), (400, 100))
        DISPLAYSURF.blit(COMIC_SANS.render(
            ' '.join(game_vars['word_to_guess_display']), True, BLACK), (500, 400))
        ...
        DISPLAYSURF.blit(pygame.image.load("hangman.png").subsurface(
            game_vars["sub_surface"]), (200, 250))
        DISPLAYSURF.blit(COMIC_SANS.render(
            "Difficulty = Easy", True, YELLOW), (400, 10))
        DISPLAYSURF.blit(COMIC_SANS.render(
            "Guess the word!", True, GREEN), (400, 100))
        DISPLAYSURF.blit(COMIC_SANS.render(
            ' '.join(game_vars['word_to_guess_display']), True, BLACK), (500, 400))
...
while running:
    if toggle_add == False:
        add_new_word_button_text = COMIC_SANS.render(
            "Add new word", True, WHITE)
        add_new_word_button_rect_center = add_new_word_button_text.get_rect(
            center=add_new_word_button_rect.center)
    ...
    if check_loose(game_vars):
        if int(score_calculate(game_vars['score'], int(time.time()-start), SCORE_COEF[state])) > int(Hard_scoreboard_data[[i for i in Hard_scoreboard_data][2]]):
            scoreboard[state][def_user] = int(score_calculate(
                game_vars['score'], int(time.time()-start), SCORE_COEF[state]))
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if state == "main_menu":
                if add_new_word_button_rect.collidepoint(event.pos):
                    title_better_luck_next_time_text = COMIC_SANS.render(
                        end_messages[99], True, BLACK)
                    title_better_luck_next_time_rect_center = title_better_luck_next_time_text.get_rect(
                        center=title_better_luck_next_time_rect.center)
                    active = False
                    toggle_add = True
                    add_new_word_button_text = COMIC_SANS.render(
                        ' '.join(typed_word), True, WHITE)
                    add_new_word_button_rect_center = add_new_word_button_text.get_rect(
                        center=add_new_word_button_rect.center)
                ...
                if exit_button_rect.collidepoint(event.pos):
                    ...
                    game_vars["word_to_guess_display"] = [
                        "_"] * len(game_vars["word_to_guess"])
                    ...
                    Easy_scoreboard_data = sort_score_board(
                        scoreboard[([i for i in scoreboard])[0]])
                    Medium_scoreboard_data = sort_score_board(
                        scoreboard[([i for i in scoreboard])[1]])
                    Hard_scoreboard_data = sort_score_board(
                        scoreboard[([i for i in scoreboard])[2]])
                    state = "scoreboard"
            elif state == "scoreboard":
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if toggle_add:
                if event.unicode == '\x08':
                    ...
                    typed_word = [i for i in add_word_temp] + \
                        ['_']*(len(typed_word)-len(add_word_temp))
                    add_new_word_button_text = COMIC_SANS.render(
                        ' '.join(typed_word), True, WHITE)
                    add_new_word_button_rect_center = add_new_word_button_text.get_rect(
                        center=add_new_word_button_rect.center)
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    add_new_word_button_rect_center = add_new_word_button_text.get_rect(
                        center=add_new_word_button_rect.center)
                    title_better_luck_next_time_text = COMIC_SANS.render(
                        end_messages[80], True, BLACK)
                    title_better_luck_next_time_rect_center = title_better_luck_next_time_text.get_rect(
                        center=title_better_luck_next_time_rect.center)
                    toggle_add = False
            if active:
                input_name_box_text = COMIC_SANS.render(
                    'Please enter your name: '+(' '.join(visual_text)), True, BLACK)
                if event.key == pygame.K_LSHIFT or event.key == pygame.K_RSHIFT:
                    continue
                input_name_box_text = COMIC_SANS.render(
                    'Please enter your name: '+(' '.join(visual_text)), True, BLACK)
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    active = not active
                    def_user = input_name
                    input_name_box_text = pygame.font.SysFont('Comic Sans MS', 30).render(
                        ('Welcome to Hangman '+''.join(def_user))+'!', True, BLACK)
                elif event.key == pygame.K_BACKSPACE:
                    visual_text = list(
                        (''.join(visual_text)).replace('\x08', '_'))
            ...
            if not active:
                if text_input_output == '':
                    if (count % 2 == 0):
                        ...
                        input_name_box_text = COMIC_SANS.render(
                            'Please enter your name: ', True, BLACK)
                    count += 1
            sub_surface[0] += 200
